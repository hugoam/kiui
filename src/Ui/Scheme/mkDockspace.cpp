...
namespace mk
{
	Docksection::Docksection(Dockline* dockline, size_t index, Style* style)
		: Tabber(style ? style : styleCls(), true)
		, mDockline(dockline)
		, mIndex(index)
	{
		mType = cls();
	...
	Dockline::Dockline(Dockspace* dockspace, Dockline* dockline, Dimension dim, size_t index, Style* style)
		: GridSheet(dim, style)
	...
	Dockline* Dockline::insertLine(size_t index, bool replace, float span)
	{
		...
		if(mDim == DIM_X)
			dockline = this->makeinsert<DocklineY>(index, mDockspace, this, index);
		else
	...
	Docksection* Dockline::insertSection(size_t index, Style* style, float span)
	{
		Docksection* docksection = this->makeinsert<Docksection>(index, this, index, style);
	...
	Docksection* Dockline::findOrCreateSection(const string& dockid, Style* style)
	{
